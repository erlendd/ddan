...
class DANNModel(object):
    ...
    def __init__(self, nfeatures=50, arch_shared=[32, 'act'], arch_domain=[8, 'act'], arch_clf=[], 
        ...
        activations='relu', epochs=1000, optimizer=None, noise=0.0, droprate=0.0):
    ...
    def _build(self, input_layer, arch, activations, noise, droprate):
        print 'Building network layers...'
        network = [input_layer]
        for nunits in arch:
            print nunits
            if isinstance(nunits, int):
                network += [Dense(nunits, activation='linear', kernel_initializer='glorot_uniform')(network[-1])]
    ...
    def _build_model(self, nfeatures, arch_shared, arch_domain, arch_clf, 
        ...
        full_feat = self._build(self.X, arch_shared, activations, noise, droprate)
        ...
        y_net = self._build(select_feat, arch_clf, activations, noise, droprate)
        y_logits = Dense(1, activation='linear')(y_net[-1])
        self.y_clf = Activation('sigmoid')(y_logits)

        domain_net = self._build(grl, arch_domain, activations, noise, droprate)
        domain_logits = Dense(1, activation='linear')(domain_net[-1])
        ...
        self.domain_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.domain, logits=domain_logits))
